...
describe("Tunnelmole integration tests", () => {
  ...
  beforeAll(async () => {
    ...
    if (availablePort !== port) {
      console.log(
        `Port 3001 is already in use. Tests require this port, so exiting tests. Free up this port before running the tests again`,
      );
      process.exit(1);
    }
  ...
  it("Upload binary photo", async () => {
    const image: Buffer = fs.readFileSync(
      `${ROOT_DIR}/test/integration/files/img/test-image.png`,
    );
  ...
  it("Submit multipart/form-data with image and text field as POST", async () => {
    ...
    const response = await fetch(
      url + "/post-submit-form-multipart-with-image",
      {
        method: "POST",
        body: form,
      },
